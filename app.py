...
# --- CONFIGURAZIONE ---
st.title("📅 Gestione Turni - Comunità")
...
with c2:
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        df_export.to_excel(writer, sheet_name='Turni_Settimanali')
    
    st.download_button(
        label="📥 SCARICA EXCEL PER WHATSAPP",
        data=output.getvalue(),
        file_name=f"turni_comunita_{datetime.now().strftime('%Y%m%d')}.xlsx",
